...
<head>
  ...
  <script nonce="61bd59ad-7e7a-4398-b900-881e8d6e9e0d">
    (function(w, d) {
      ! function(a, e, t, r, z) {
              ...
              <div class="card-body">
                ...
                <div id="cari_nama" style="overflow-x:auto;">
                  <table id="tb_kamar" class="table table-striped" style="width:100%">
                    ...
                    <tbody>
                      <?php
                      $sql = "SELECT * FROM tb_pelanggan JOIN tb_kamar ON tb_pelanggan.id_kamar = tb_kamar.id_kamar WHERE status IN (0,1,2) ORDER BY tb_pelanggan.tgl_pesan DESC LIMIT 20";
                      $result = $conn->query($sql);
                      if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                          if ($row["status"] == "1") {
